...
async def _on_message(update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # Note: media (photos, voice, docs) could be handled here with download +
    # upload to AnythingLLM, but keeping v1 text-only for simplicity.
    if message.photo:
        content_parts.append("[image attached — not yet supported by bridge]")
    if message.voice or message.audio:
        content_parts.append("[audio attached — not yet supported by bridge]")
    if message.document:
        content_parts.append("[document attached — not yet supported by bridge]")
